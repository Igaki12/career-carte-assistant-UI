...
const SHIRP_GUIDE = `
S (Satisfaction/現状): 組織適応 / 自身への評価 / 良好な人間関係 / そのほかの現状
H (Hope/希望): 希望する収入 / 希望する仕事内容 / 希望する勤務形態 / そのほかの希望
I (Issue/課題): スキル / 健康 / 年齢 / 家庭 / そのほかの課題
R (Resource/資源): 資格 / 経験 / 協力者 / 時間や資金 / そのほかの強み
P (Plan/プラン): S〜Rを踏まえた解決プラン
# (その他): S〜Pに当てはまらない自由記述
`;

const greetingForMeeting = (meetingType: MeetingType) =>
  meetingType === 'initial'
    ? 'こんにちは。キャリアメンターです。まずは現状について伺います。今の仕事で満足している点や、気になっている点を教えてください。'
    : 'こんにちは。キャリアメンターです。今日は自由にお話しください。必要に応じて、前回のカルテ内容を更新していきます。';
...
const MeetingRoom = ({ meetingType, continuousMode = 'normal' }: Props) => {
  ...
  const handleSubmitKarte = useCallback(() => {
    ...
    persistKarte();
    setKarteModalOpen(false);
  }, [disposeActiveAudio, persistKarte]);
