...
- DemographicsSetup (`/user/demographics`): プロフィール初期設定・編集。未設定時の導線兼、UserHomeからの編集画面。`ID`, `メール`, `姓`, `名`, `フリガナ（姓）`, `フリガナ（名）`, `会社名` は表示するが編集不可にし、白背景ではなく固定項目と分かる暗色の読取専用スタイルで表示する。メールはIDの次、姓の前に配置する。入力欄の placeholder は例示として `例: ...` を付ける。生年月日はブラウザ標準の `type="date"` で入力し、保存値は `YYYY-MM-DD` に正規化する。勤務地(都道府県)は北海道から沖縄県までの47都道府県セレクトにする。職種は `src/lib/jobTitles.ts` の候補（営業職、事務/管理職、販売/サービススタッフ職、クリエイティブ職、医療/福祉職、専門/資格（弁護士等）職、IT/エンジニア職、製造/技術職、物流/軽作業職、公務/教育職、その他）から選択する。転職歴、勤続年数、末子年齢などの数値欄は数字のみ入力可能にする。デモ検証用に「デモ用にスキップして進む」を持つ。
- UserHome (`/user`): 一般ユーザー・企業管理者共通のマイページ。初期版では面談、カルテ閲覧・出力、プロフィール編集を主要導線とする。面談カードの見出しは「面談」、初回面談へ進む主要ボタンとAuthNavigation内のリンクは「キャリアカルテの作成」と表示する。初回面談の下書きラベルはユーザー向けには「前回面談の下書き」と表示する。継続面談、ユーザーアンケート、面談前コンディションチェックは準備中として disabled または非表示にし、開始できない状態にする。ステータスは summary 表示を基本とし、詳細 badge の羅列は表示しない。基本アカウント情報は `ID`, `メール`, `氏名`, `フリガナ`, `会社名`, `部署`, `職種` の順に表示する。アカウント情報確認内の操作ボタンは幅いっぱい、文字中央揃えで表示する。カルテ出力は最新の保存済みカルテ1件を対象に、ブラウザ経由で CSV / PDF を直接ダウンロードできるようにする。一般ユーザー画面では面談利用回数、残り回数、AI利用可能回数を表示しない。企業管理者権限のユーザーには、マイページ下部に企業管理者画面へ移動するカードを表示する。
- ConsultantHome (`/consultant`): コンサルタント用。担当ユーザーのカルテ閲覧・修正、AI練習面談（ロードマップ）。コンサルタント概要、アカウント情報確認、カルテ内プロフィールなどの基本アカウント情報は `ID`, `メール`, `氏名` を先頭順に表示し、アカウント操作ボタンは幅いっぱい、文字中央揃えで表示する。
- Admin (`/admin`): システム管理者用。アカウント管理、企業別オプション管理、企業テナントごとの企業API総回数・企業API使用済み回数・面談1回あたり最大ターン数の設定を行う。企業API項目は個別ユーザーではなく企業単位で管理し、企業管理者画面からは変更させない。個別追加・編集・一括編集の職種は `src/lib/jobTitles.ts` の候補から選択する。
- CompanyAdminHome (`/company-admin`): 企業管理者用。自社テナントの従業員カルテ一覧、検索、並び替え、一括選択、カルテ表示、個別/一括PDF出力、個別/一括印刷、企業API使用状況・残り面談回数・面談1回あたり最大ターン数の確認、コンディション測定件数の集計表示を行う。企業API総回数・使用済み回数・最大ターン数・企業別オプションはシステム管理者画面で変更するため、企業管理者画面では編集UIを持たない。
...
- 基本アカウント情報の表示順は画面間で揃え、`ID`, `メール`, `氏名`, `フリガナ`, `会社名`, `部署`, `職種` を基本順とする。カード内の主要操作ボタンは、狭い画面で欠けないよう原則として幅いっぱい、文字中央揃えで表示する。
...
- MeetingRoom の下部操作行は2列構成とし、左に左矢印アイコン付きの「中断して戻る」、右に「カルテを確認」を配置する。
...
- 例外: デモ版の企業API使用枠・会話ターン制限は `src/lib/demoUsageQuota.ts` のテナント別メモリ状態を正とし、`localStorage` へ保存しない。システム管理者画面 `/admin` の企業別オプション管理から企業単位で変更し、SPA内の画面遷移では同期し、ページリフレッシュ時はデフォルト値へ戻す。
- デモ用クォータ初期値:
  - 企業API総回数: 1000回
  - 企業API使用済み回数: 0回
  - 面談1回あたり最大ターン数: 100回
...
- 会話ターン制限: 初回面談のAI使用可能回数は、デフォルトでは面談1回あたり100回とする。システム管理者画面 `/admin` の企業別オプション管理でテナントごとに変更した値を `MeetingRoom` の送信可能回数に連動させる。
...
- 会話ターン制限: 継続面談のAI使用可能回数は、初回面談と同じ企業別の最大ターン数を使う。デフォルトでは面談1回あたり100回とし、システム管理者画面 `/admin` の企業別オプション管理でテナントごとに変更した値を `MeetingRoom` の送信可能回数に連動させる。
...
- 生年月日は `type="date"`、転職歴・勤続年数・末子年齢などの数値項目は数字のみ入力できるテキスト入力として扱う。
- 性別は `男 / 女 / その他` の選択式とする。
...
## 7. 管理・バックエンド仕様と将来展望
- 利用回数管理UI: 企業API項目は企業単位で扱い、システム管理者画面 `/admin` の企業別オプション管理で `企業API総回数 / 企業API使用済み回数 / 残り / 面談1回あたり最大ターン数` を変更できるようにする。CompanyAdminHomeでは自社テナントの残り面談回数と使用状況を確認のみ可能にする。UserHomeでは利用回数・残り回数を表示しない。
- 会話ターン制限UI: システム管理者画面 `/admin` では、表記を `面談1回あたり最大ターン数` とし、企業テナントごとに `MeetingRoom` の送信可能回数と連動させる。CompanyAdminHomeでは同値を確認のみ可能にし、UserHomeではAI利用可能回数を表示しない。
...
- 企業管理者画面: `/company-admin` では自社テナントの企業API使用状況、残り面談回数、面談1回あたり最大ターン数を確認できるが、企業API項目と企業別オプションは変更できない。加えて、自社テナントの従業員カルテを検索・並び替え・一括選択し、個別/一括PDF出力と個別/一括印刷を行える。個人別の顔分析・緊張度詳細は表示せず、測定件数などの集計値に留める。
