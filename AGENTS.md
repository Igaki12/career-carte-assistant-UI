...
- 基本アカウント情報の表示順は画面間で揃え、ユーザー本人向け画面では `氏名`, `フリガナ`, `メール`, `会社名`, `部署`, `職種` を基本順とし、内部IDは表示しない。姓名分割フォームでは `姓`, `名`, `フリガナ（姓）`, `フリガナ（名）`, `メール`, `会社名` の順にする。管理系画面では紐づけ・監査・検索・ソートのため `ID` 表示を許可する。カード内の主要操作ボタンは、狭い画面で欠けないよう原則として幅いっぱい、文字中央揃えで表示する。
...
1. プロフィール（個人情報）
- 基本情報はユーザー本人向けには `氏名`, `フリガナ`, `メール`, `会社名`, `部署`, `職種` の順で表示し、管理系画面では必要に応じて `ID` も表示する。
- 氏名は保存上 `姓` / `名`、フリガナは `フリガナ（姓）` / `フリガナ（名）` を正とし、表示・出力時に結合して扱う。
- `ID` / `accountId` は内部管理用の自動生成IDとして保持するが、ユーザー本人向け画面では表示・編集させない。`姓`, `名`, `フリガナ（姓）`, `フリガナ（名）`, `メール`, `会社名` はユーザー画面では編集不可にする。
