...
- 管理コンソールでは **アカウント単位で面談回数・LLM使用回数を設定**する方針に移行し、tier 管理 UI は廃止した。
...
| カルテ UI (7スロット) | ✅ | モーダルで最新カルテを詳細表示・編集可能。過去カルテは作成日/ステータス/Aのみ簡易表示。ステータスは「作成途中 / 作成済み / ユーザー編集済み / コンサル編集済み」 |
| ユーザーアンケート | ✅ | 25問構成（全問リッカート形式）。前回スコア表示と「前回回答を編集する」ボタンあり。1問4点換算で100点満点に正規化 |
| VRM ステージ | ✅ | 背景切替・Blink/Nod/Surprised 制御・進捗バッジ重ね表示。低スペック端末の FPS に注意 |
| 管理コンソール（アカウント管理） | ✅ | ユーザー/コンサルの一覧にチェックボックス選択・全件選択を追加。面談回数（残り/上限）と LLM 使用回数/面談を列で表示 |
| 管理コンソール（編集UI） | ✅ | 行単位の編集モーダルと一括編集モーダルを実装。選択件数に応じて「全て編集/削除」を表示し、パスワード再発行（ダミー）も提供 |
...
5. **管理コンソールの拡張**  
   - 一括削除に確認ダイアログを追加し、面談回数のリセット/プリセット操作を検討する。
