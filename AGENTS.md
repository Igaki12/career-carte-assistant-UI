...
1. プロフィール（個人情報）
- 基本情報は `ID`, `メール`, `氏名`, `フリガナ`, `会社名`, `部署`, `職種` の順で表示する。
- 氏名は保存上 `姓` / `名`、フリガナは `フリガナ（姓）` / `フリガナ（名）` を正とし、表示・出力時に結合して扱う。
- `ID`, `メール`, `姓`, `名`, `フリガナ（姓）`, `フリガナ（名）`, `会社名` はユーザー画面では編集不可にする。
