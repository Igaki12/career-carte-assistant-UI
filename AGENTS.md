...
### 2.2 役割別ページ構成とルーティング
- Login (`/login`): 一般ユーザー・企業管理者・運用管理者・キャリアコンサルタント共通のダミーログイン画面。入力されたメールアドレスを `demo-accounts.json` の `email` と完全一致照合し、該当アカウントのロールで認証する。通常ログイン画面ではアカウントID入力、ロール選択、利用条件モーダル、利用条件同意チェック、システム管理者リンク、デモ説明文は表示しない。企業管理者も一般ユーザーと同じ入口を使い、ログイン後は `/user` に遷移する。運用管理者は `/operations-admin` に遷移する。常にログインした状態にしておくチェックを持つ。
...
- DemographicsSetup (`/user/demographics`): プロフィール初期設定・編集。未設定時の導線兼、UserHomeからの編集画面。ユーザーIDは内部管理用の自動生成IDであり、ユーザー本人向け画面には表示しない。`メール`, `姓`, `名`, `フリガナ（姓）`, `フリガナ（名）`, `会社名` は表示するが編集不可にし、白背景ではなく固定項目と分かる暗色の読取専用スタイルで表示する。メールは先頭、姓の前に配置する。`権限` は表示しない。入力欄の placeholder は例示として `例: ...` を付ける。生年月日はブラウザ標準の `type="date"` で入力し、保存値は `YYYY-MM-DD` に正規化する。勤務地(都道府県)は北海道から沖縄県までの47都道府県セレクトにする。職種は `src/lib/jobTitles.ts` の候補（営業職、事務/管理職、販売/サービススタッフ職、クリエイティブ職、医療/福祉職、専門/資格（弁護士等）職、IT/エンジニア職、製造/技術職、物流/軽作業職、公務/教育職、その他）から選択する。転職歴、勤続年数、末子年齢などの数値欄は数字のみ入力可能にする。マネージャー経験項目（過去にマネージャー経験があるか、現在マネージャーか）を持つ。婚姻関係、子供人数、末子年齢、マネージャー経験系の選択肢には回答しない選択肢を用意する。デモ検証用に「デモ用にスキップして進む」を持つ。
- UserHome (`/user`): 一般ユーザー・企業管理者共通のマイページ。初期版では面談、カルテ閲覧・出力、プロフィール編集を主要導線とする。面談カードの見出しは「面談」、説明文は `面談を実施し、キャリアカルテを作成しましょう。※途中での中断も可能です`、初回面談へ進む主要ボタンとAuthNavigation内のリンクは「キャリアカルテの作成」と表示する。初回面談の下書きラベルはユーザー向けには「前回面談の下書き」と表示する。継続面談、ユーザーアンケート、面談前コンディションチェックは初期版では画面または導線ごと非表示にする。ステータスは summary 表示を基本とし、詳細 badge の羅列は表示しない。基本アカウント情報は `メール`, `氏名`, `フリガナ`, `会社名`, `部署`, `職種` の順に表示し、ユーザーIDは表示しない。プロフィール右側の肩書きなど属性情報だけを強調する表示は出さない。アカウント情報確認内の操作ボタンは幅いっぱい、文字中央揃えで表示する。カルテ出力は最新の保存済みカルテ1件を対象に、ブラウザ経由で CSV / PDF を直接ダウンロードできるようにする。一般ユーザー画面では面談利用回数、残り回数、AI利用可能回数を表示しない。企業管理者権限のユーザーには、マイページ下部に企業管理者画面へ移動するカードを表示する。
- ConsultantHome (`/consultant`): コンサルタント用。担当ユーザーのカルテ閲覧・修正、AI練習面談（ロードマップ）。コンサルタント本人向けの概要・アカウント情報確認では内部IDを表示せず、`メール`, `氏名` を先頭順に表示し、アカウント操作ボタンは幅いっぱい、文字中央揃えで表示する。担当ユーザーの管理系一覧では必要に応じてID表示を許可する。
- Admin (`/admin`): システム管理者用。アカウント管理、企業別オプション管理、企業テナントごとの企業API総回数・企業API使用済み回数・面談1回あたり最大ターン数の設定を行う。企業API項目は個別ユーザーではなく企業単位で管理し、企業管理者画面からは変更させない。ユーザー管理では氏名を `姓` / `名`、フリガナを `フリガナ（姓）` / `フリガナ（名）` に分け、企業紐づけ、検索、ソート、削除、カルテCSV/印刷に対応する。個別追加・編集・一括編集の職種は `src/lib/jobTitles.ts` の候補から選択する。
- OperationsAdminHome (`/operations-admin`): 運用管理者用。企業管理者より上位、システム管理者より下位の顧客側複数企業横断管理者として、管理対象企業の企業別状況と従業員カルテ一覧を確認する。`managedTenantIds` に含まれる企業のみを対象にし、企業名、従業員数、カルテ保存済み件数、残り面談回数、使用状況、面談1回あたり最大ターン数、ユーザー所属企業、カルテ作成状況を表示する。ユーザー一覧は企業名で検索・ソートでき、保存済みカルテは表示、CSV出力、PDF出力できる。企業API総回数・使用済み回数・最大ターン数・企業別オプションはシステム管理者画面で変更するため、運用管理者画面では編集UIを持たない。
- CompanyAdminHome (`/company-admin`): 企業管理者用。自社テナントの従業員カルテ一覧、検索、並び替え、一括選択、カルテ表示、個別/一括PDF出力、個別/一括印刷、企業API使用状況・残り面談回数・面談1回あたり最大ターン数の確認を行う。自社従業員カルテ一覧には `フリガナ` とキャリアカルテ作成完了/未完了状況を表示する。企業API総回数・使用済み回数・最大ターン数・企業別オプションはシステム管理者画面で変更するため、企業管理者画面では編集UIを持たない。初期版ではパスワード通知文一覧、社員登録画面、企業別オプション、コンディション測定件数、緊張度スコア表示を表示しない。
...
- ロール別にアクセス可能画面を分離する。`user` は `/user`, `/user/demographics`, `/user/condition-check`, `/app/initial`, `/app/continuous`、`company-admin` は `/user`, `/user/demographics`, `/user/condition-check`, `/app/initial`, `/app/continuous`, `/company-admin`、`operations-admin` は `/operations-admin`、`consultant` は `/consultant`、`admin` は `/admin` を対象とする。
- ログイン済みの保護ページ右上には `src/components/AuthNavigation.tsx` の開閉式ナビゲーションを表示する。デフォルトは歯車アイコンボタンの縮小表示とし、初回表示時は展開状態から短時間で縮小して、ナビゲーションの存在が分かるアニメーションを入れる。展開時は `Career Karte Assistant`、ログインメールアドレス、ロール、権限内ページリンク、ログアウトボタンを表示する。
...
- 基本アカウント情報の表示順は画面間で揃え、ユーザー本人向け画面では `メール`, `氏名`, `フリガナ`, `会社名`, `部署`, `職種` を基本順とし、内部IDは表示しない。管理系画面では紐づけ・監査・検索・ソートのため `ID` 表示を許可する。カード内の主要操作ボタンは、狭い画面で欠けないよう原則として幅いっぱい、文字中央揃えで表示する。
- カルテ確認・編集、メール問い合わせなどのモーダルはページ全体の黒基調に合わせ、暗色ガラスパネル、角丸なし、上端のシルバー系グラデーションライン、白基調の本文で表示する。広い画面では用途に応じて横幅を広げ、カルテ系モーダルは最大 `1480px` 程度、問い合わせ系モーダルは最大 `1120px` 程度を目安にする。利用条件モーダルは初期版のログイン導線には表示しない。
...
- ダミーアカウント定義は `demo-accounts.json` に置く。運用管理者は `role: "operations-admin"` と `managedTenantIds` を持ち、複数企業横断の表示対象を決める。
...
- 中断動線: 下部操作行は2列構成とし、左に左矢印アイコン付きの `中断して戻る`、右に `カルテを確認` を配置する。
...
1. プロフィール（個人情報）
- 基本情報はユーザー本人向けには `メール`, `氏名`, `フリガナ`, `会社名`, `部署`, `職種` の順で表示し、管理系画面では必要に応じて `ID` も表示する。
- 氏名は保存上 `姓` / `名`、フリガナは `フリガナ（姓）` / `フリガナ（名）` を正とし、表示・出力時に結合して扱う。
- `ID` / `accountId` は内部管理用の自動生成IDとして保持するが、ユーザー本人向け画面では表示・編集させない。`メール`, `姓`, `名`, `フリガナ（姓）`, `フリガナ（名）`, `会社名` はユーザー画面では編集不可にする。
- ユーザー向けプロフィール表示では `権限` を表示しない。管理系一覧では権限表示を維持する。
- 詳細情報として `生年月日`, `勤務地(都道府県)`, `転職歴(回数)`, `勤続年数(年)`, `性別`, `現在の婚姻関係`, `子供の有無(人)`, `末子の年齢(歳)`, `過去にマネージャー経験があるか`, `現在マネージャーか` を扱う。
...
- 勤務地は北海道から沖縄県までの47都道府県セレクトとする。
- 職種は `src/lib/jobTitles.ts` のカテゴリから選択する。
- 性別は `男 / 女 / その他` の選択式とする。
- 婚姻関係は `独身 / 既婚 / その他 / 回答しない` の選択式とする。
- 子供人数は `0`, `1`, `2`, `3`, `4以上`, `回答しない` の選択式とする。
- 子供人数が `0`、未入力、または `回答しない` のとき、末子年齢は入力不可にし、保存値も `null` に正規化する。
- マネージャー経験系は `ある / ない / 回答しない` または `はい / いいえ / 回答しない` の選択式とする。
...
2. 電子カルテ（SHIRP形式）
    ...
    *   ユーザー向けCSV/PDFのプロフィール出力には `氏名`, `フリガナ`, `生年月日` を含め、`権限` は含めない。管理系一覧では権限表示を維持する。
...
4. ユーザーアンケート結果
- 初期版ではUserHome上のボタン・カード・ナビゲーション導線を非表示にする。正式有効化までは一般ユーザーが回答開始できない状態を維持する。
