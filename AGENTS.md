...
#### 3.2.1 ターンテイキングモード実装制約
- 現時点では、Realtime APIを使った「会話しながらカルテを常時更新する本実装」は未対応とする。
- 理由:
  - 公式ガイド上、ブラウザクライアントはWebSocket直結よりWebRTC + ephemeral token構成が推奨される。
  - このリポジトリにはephemeral tokenを安全に発行するバックエンドが存在しない。
  - そのため、フロント単体で安全かつ安定したRealtime接続を完結できない。
