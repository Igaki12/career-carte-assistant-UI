...
- UserHome (`/user`): 一般ユーザー用。初回/継続面談スタート、継続面談モード選択、カルテ閲覧・出力、アンケート、プロフィール編集、面談前コンディションチェック導線を配置する。ステータスは summary 表示を基本とし、詳細 badge の羅列は表示しない。カルテ出力は最新の保存済みカルテ1件を対象に、ブラウザ経由で CSV / PDF を直接ダウンロードできるようにする。
